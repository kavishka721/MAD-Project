...
public class ssdselection extends AppCompatActivity {
    ...
    private ArrayList<SSD> SSDList = new ArrayList<>();
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.hasChildren()) {
                    Log.d("Child Count :",""+dataSnapshot.child("SSD").getChildrenCount());
                    System.out.println("Hello CPUS");
                    for (DataSnapshot postSnapshot : dataSnapshot.child("SSD").getChildren()){
                        Log.d("KEY VALUE:",postSnapshot.getKey());
                        SSD ssd1 = postSnapshot.getValue(SSD.class);
                        Log.d("CPU Lists",ssd1.getName());
                        SSDList.add(ssd1);
                    }
                        Log.d("Image List", String.valueOf(mStorageRef.child("SSD Images").listAll()));
                        mStorageRef.child("SSD Images").listAll().addOnSuccessListener(new OnSuccessListener<ListResult>() {
                            @Override
                    ...
                    Recycle = new SSDRecyclerAdapter(getBaseContext(), SSDList, imageList);
    ...
    public void initImageList(final ArrayList<String> fileList){
        ...
        for (String file : fileList) {
            mStorageRef.child("SSD Images/" + file).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                @Override
